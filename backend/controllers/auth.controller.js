...
export const signupPhone = async (req, res) => {
    ...
    try {
        ...
        // Send JWT in response header instead of cookies
        const token = generateTokenAndSetHeader(res, user._id);
        ...
        res.status(201).json({
            success: true,
            message: "User created successfully",
            token, // Send token to be stored in AsyncStorage
            user: {
                ...user._doc,
                password: undefined,
            },
...
export const verifyEmailPhone = async (req, res) => {
    const { code } = req.body;
    try {
        const user = await User.findOne({
            verificationToken: code,
...
export const loginPhone = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            success: true,
            message: "Logged in successfully",
            token, // Send token for AsyncStorage
            user: {
                ...user._doc,
                password: undefined,
            },
...
export const forgotPasswordPhone = async (req, res) => {
    ...
    try {
        ...
        await sendResetPasswordEmail(user.email, user.name, `${FEHosturl}/reset-password/${resetToken}`);
...
export const checkAuthPhone = async (req, res) => {
    try {
        const user = await User.findById(req.userId).select("-password");
        if (!user) {
            return res.status(400).json({ success: false, message: "User not found" });
